...
KangarooChannel::KangarooChannel(KangarooSerial& serial, char name, uint8_t address)
...
KangarooMonitor KangarooChannel::motion(uint8_t motionType, int32_t motionValue, uint8_t limitType, int32_t limitValue,
...
KangarooError KangarooChannel::systemCommand(KangarooSystemCommand systemCommand, bool expectReply, int32_t values[], size_t valueCount)
{
    KangarooCommandWriter contents;
                                           ...
                                           const KangarooTimeout& timeout)
{
  ...
  while (1)
  {
    ...
    while (_serial.port().available() == 0); 
    int a = _serial.port().available();
...
bool KangarooChannel::getInitialSequenceCodeIfNecessary(const KangarooTimeout& timeout, KangarooStatus& status)
{
...
bool KangarooChannel::updateMonitoredResult(const KangarooTimeout& timeout,
                                               bool acceptRepliesWithStartupSequenceCode)
{
  ...
  if (_monitoredGetResult.sequenceCode() == _monitoredSequenceCode || _monitoredGetType==KANGAROO_GETP)
  {
    return true;
  }
