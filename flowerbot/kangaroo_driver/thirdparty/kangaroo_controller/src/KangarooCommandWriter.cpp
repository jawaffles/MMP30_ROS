...
void KangarooCommandWriter::write(uint8_t data)
{
  _data[_length ++] = data;
}
...
{
  if (number>KANGAROO_BIT_PACKED_MAX_VALUE)
...
size_t KangarooCommandWriter::writeToBuffer(uint8_t* buffer, uint8_t address, KangarooCommand command) const
{
  return writeToBuffer(buffer, address, command, data(), length());
}

size_t KangarooCommandWriter::writeToBuffer(uint8_t* buffer, uint8_t address, KangarooCommand command, const uint8_t* data, size_t lengthOfData)
{
  size_t i = 0;
...
void KangarooCommandWriter::writeToStream(Stream& port, uint8_t address, KangarooCommand command) const
{
  writeToStream(port, address, command, data(), length());
}

void KangarooCommandWriter::writeToStream(Stream& port, uint8_t address, KangarooCommand command, const uint8_t* data, size_t lengthOfData)
{
  ...
  port.writeBytes(buffer, lengthOfBuffer);
}
