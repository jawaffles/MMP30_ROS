...
bool KangarooReplyReader::canRead() const
{
...
{
  uint8_t data = *_data; _data ++; return data;
}

int32_t KangarooReplyReader::readBitPackedNumber()
{
	uint32_t encodedNumber = 0; uint8_t shift = 0;

	for (uint8_t i = 0; i < 5; i ++) // 30 bits
	{
		uint8_t uint8_t;
		if (!tryRead(&uint8_t)) { break; }
		
		encodedNumber |= (uint32_t)(uint8_t & 0x3f) << shift; shift += 6;
		if (!(uint8_t & 0x40)) { break; }
	}
