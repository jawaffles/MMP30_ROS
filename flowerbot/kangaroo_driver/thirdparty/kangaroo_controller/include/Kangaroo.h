...
public:
         size_t writeToBuffer(uint8_t* buffer, uint8_t address, KangarooCommand command) const;
  static size_t writeToBuffer(uint8_t* buffer, uint8_t address, KangarooCommand command, const uint8_t* data, size_t lengthOfData);
         void   writeToStream(Stream& port, uint8_t address, KangarooCommand command) const;
  static void   writeToStream(Stream& port, uint8_t address, KangarooCommand command, const uint8_t* data, size_t lengthOfData);
  
private:
  uint8_t _address, _command; size_t _length;
...
public:
  ...
  */
   KangarooChannel(KangarooSerial& serial, char name, uint8_t address = KANGAROO_DEFAULT_ADDRESS);
  ~KangarooChannel();
...
public:
  ...
  */
  ...
  \return A KangarooError.
  */
  KangarooError systemCommand(KangarooSystemCommand systemCommand, bool expectReply, int32_t values[], size_t valueCount);

private:
  KangarooStatus  getSpecial(KangarooGetType type, KangarooGetFlags flags, const KangarooTimeout& timeout);
  KangarooMonitor motion    (uint8_t motionType, int32_t motionValue, uint8_t limitType, int32_t limitValue, KangarooMoveFlags flags);
  ...
  bool getInitialSequenceCodeIfNecessary(const KangarooTimeout& timeout, KangarooStatus& status);
  bool updateMonitoredResult(const KangarooTimeout& timeout, bool acceptRepliesWithStartupSequenceCode);
...
*/
bool waitAll(size_t count, KangarooMonitor* monitors[], int32_t timeoutMS = KANGAROO_INFINITE_TIMEOUT);
...
\remark If a monitor is null, it is ignored.
*/
bool waitAll(size_t count, KangarooMonitor* monitors[], const KangarooTimeout& timeout);
